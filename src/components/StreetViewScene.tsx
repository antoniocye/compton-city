...
function makePinEl(
  ...
  const ring  = isDark ? "rgba(245,158,11,0.35)"  : "rgba(217,119,6,0.30)";
  const dot   = isDark ? "#f59e0b" : "#d97706";
  const chipBg= isDark ? "#18181b"                 : "#ffffff";
  const chipTx= isDark ? "#f4f4f5"                 : "#18181b";
  const chipBr= isDark ? "#3f3f46"                 : "#d4d4d8";
  ...
  const countBadge = summary.artifactCount > 1
    ? `<span style="
        ...
        padding:2px 6px;border-radius:999px;font-size:9px;font-weight:800;
        letter-spacing:.12em;text-transform:uppercase;
        color:${chipTx};background:${isDark ? "#27272a" : "#f4f4f5"};
...
export default function StreetViewScene({
  ...
  const txt  = isDark ? "text-zinc-100" : "text-zinc-800";
  const sub  = isDark ? "text-zinc-400" : "text-zinc-500";
  const code = isDark ? "bg-zinc-800 text-amber-300 border border-zinc-700" : "bg-zinc-100 text-amber-700 border border-zinc-200";
  const link = isDark
    ? "text-amber-400 hover:text-amber-300 underline underline-offset-2 transition-colors"
    : "text-amber-600 hover:text-amber-500 underline underline-offset-2 transition-colors";
  ...
  const Step = ({ n, children }: { n: number; children: React.ReactNode }) => (
    <div className="flex items-start gap-3 text-left">
      <span className={`shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[11px] font-bold mt-0.5 ${
        isDark ? "bg-zinc-700 text-zinc-300" : "bg-zinc-200 text-zinc-700"}`}>{n}</span>
    ...
    return (
      <div className={`w-full h-full flex items-center justify-center ${bg}`}>
        <div className="max-w-xs text-center space-y-4 px-6">
          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mx-auto ${isDark ? "bg-zinc-800 border border-zinc-700" : "bg-zinc-200 border border-zinc-300"}`}>
            <svg className={`w-7 h-7 ${isDark ? "text-zinc-500" : "text-zinc-400"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {state === "loading" && (
        <div className={`absolute inset-0 flex flex-col items-center justify-center gap-4 z-10 ${bg}`}>
          <div className="relative w-10 h-10">
            <div className={`absolute inset-0 rounded-full border-2 ${isDark ? "border-zinc-700" : "border-zinc-300"}`} />
